            ...
            PackageInstaller.STATUS_SUCCESS -> {
                ...
                val userMessage = when (status) {
                    ...
                    PackageInstaller.STATUS_FAILURE_CONFLICT -> {
                        ...
                        val installedHere = !targetPackage.isNullOrBlank() &&
                            ApkManager.getInstalledAppInfo(context, targetPackage).isInstalled
                        if (elsewhere) {
                            ApkManager.OTHER_PROFILE_CONFLICT_MESSAGE
                        } else if (installedHere || message?.contains("sign", ignoreCase = true) == true) {
                            ApkManager.SIGNATURE_CONFLICT_MESSAGE
                        } else {
                            message?.takeIf { it.isNotBlank() }
                                ?: "Install conflict: another package on this device blocks this installation."
                        }
                    }
